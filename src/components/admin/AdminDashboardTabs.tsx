...
export function AdminDashboardTabs({
  ...
  return (
    <section className="mt-8 mb-5 overflow-hidden rounded-[1.75rem] border border-white/10 bg-[#060616]/85 p-3 shadow-[0_0_40px_rgba(34,211,238,0.06)]">
      <div className="mb-3 flex flex-col gap-2 px-1 sm:flex-row sm:items-center sm:justify-between">
        ...
        <div className="rounded-full border border-white/10 bg-black/25 px-3 py-1.5 text-[10px] font-black uppercase tracking-[0.18em] text-slate-400">
          Operator Navigation
        </div>
      </div>
      ...
      <div className="relative">
        ...
        <nav className="flex gap-2 overflow-x-auto pb-1 [-ms-overflow-style:none] [scrollbar-width:none] lg:flex-wrap lg:overflow-visible lg:pb-0 [&::-webkit-scrollbar]:hidden">
          {tabs.map((tab) => {
            const Icon = tab.icon;
            ...
            return (
              <button
                ...
                className={`group relative inline-flex shrink-0 items-center gap-2 rounded-2xl border px-3.5 py-3 text-xs font-black transition sm:text-sm ${
                  isActive
