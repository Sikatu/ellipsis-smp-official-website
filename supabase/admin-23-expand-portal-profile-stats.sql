-- Unified player portal (/account) needs the richer combat/progression
-- stats that previously only the separate /profile page showed. Those
-- columns already exist on minecraft_player_profiles (added by
-- admin-9-player-profile-system.sql) -- this just extends the
-- security-definer RPC to also return them.
...
as $function$
  select
    ...
    profile.deaths,
    profile.mob_kills,
    profile.blocks_broken,
    profile.blocks_placed,
    profile.active_world,
